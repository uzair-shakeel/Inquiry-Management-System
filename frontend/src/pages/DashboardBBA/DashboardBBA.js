...
const DashboardBBA = ({ formDataList }) => {
    const bbaData = formDataList.filter(data => data.department === 'BBA');

    const totalInquiries = bbaData.length;
    const pendingInquiries = bbaData.filter(data => !data.resolved).length; // Assuming there's a resolved field
    const resolvedInquiries = bbaData.filter(data => data.resolved).length; // Assuming there's a resolved field

    return (
        <div className="min-h-screen">
            <div className="bg-white p-6 rounded-lg shadow-2xl">
                <h2 className="text-3xl font-bold mb-8 text-blue-600 text-center">BBA Department Inquiries</h2>
