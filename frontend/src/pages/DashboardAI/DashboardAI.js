...
const DashboardAI = ({ formDataList }) => {
    const aiData = formDataList.filter(data => data.department === 'AI');

    const totalInquiries = aiData.length;
    const pendingInquiries = aiData.filter(data => !data.resolved).length; // Assuming there's a resolved field
    const resolvedInquiries = aiData.filter(data => data.resolved).length; // Assuming there's a resolved field

    return (
        <div className="min-h-screen">
            <div className="bg-white p-6 rounded-lg shadow-2xl">
                <h2 className="text-3xl font-bold mb-8 text-blue-600 text-center">AI Department Inquiries</h2>
