...
const DashboardEnglish = ({ formDataList }) => {
    const englishData = formDataList.filter(data => data.department === 'English');

    const totalInquiries = englishData.length;
    const pendingInquiries = englishData.filter(data => !data.resolved).length; // Assuming there's a resolved field
    const resolvedInquiries = englishData.filter(data => data.resolved).length; // Assuming there's a resolved field

    return (
        <div className="min-h-screen">
            <div className="bg-white p-6 rounded-lg shadow-2xl">
                <h2 className="text-3xl font-bold mb-8 text-blue-600 text-center">English Department Inquiries</h2>
